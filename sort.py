...
Мы n раз вызываем функцию поиска минимума, эта функция работает за время n, так как на каждой итерации список,
 в котором мы ищем минимум скоращается на один, мы получаем что сумма времен всех вызовов будет равна сумме
  арифметической прогрессии (n + 1)*n/2 = (n^2 + n)2, что в целом учитывая тот факт, что на самом деле операций
  больше чем n в несколько раз то 2 мы можем спокойно опустить, и получим сложность алгоритма О(n^2). В худшем случае
    и в лучшем случае время работы остается одинаковым, так как поиск минимума всегда будет идти за одно и то же время.
'''
